...
import com.hdvideoplayer.databinding.AvMainItemBinding
import com.hdvideoplayer.databinding.DetailsViewBinding
import com.hdvideoplayer.databinding.RenameFieldBinding
import com.hdvideoplayer.databinding.VideoMoreFeatures2Binding
...
class VideoAdapter(
    ...
    class MyHolder(binding: AvMainItemBinding) : RecyclerView.ViewHolder(binding.root) {
...
//        val size = binding.size
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyHolder {
        return MyHolder(AvMainItemBinding.inflate(LayoutInflater.from(context), parent, false))
    }
...
//        holder.size.text = Formatter.formatShortFileSize(context, videoList[position].size.toLong())
        Glide.with(context)
            .asBitmap()
            .load(Uri.parse(videoList[position].artUri))
